...
void WebUIController::sendFile(File file, char *contenttype, bool addGzHeader, bool cash)
{
	String header;
	...
	if (cash)
		...
		sendHeader("Cache-Control", "no-cache", true);
...
{
	webServer.console->println(webServer.uri());
	if (captivePortal())
...
bool WebUIController::handleFileRead(String path, bool html, bool cash)
{
	if (path.endsWith("/"))
		path += "index.html";
	...
	if (LittleFS.exists(minimized))
		path = minimized;
	webServer.console->println("path=" + path);
	if (LittleFS.exists(path))
	{
		File file = LittleFS.open(path, "r");
		webServer.sendFile(file, contentType, false, cash);
		file.close();
		return true;
	}
	webServer.console->println("Not found!!! " + path);
	return false;
}
