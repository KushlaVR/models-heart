...
void PhysicsEffects::begin()
{
	start = millis();
	halfProgress = fullProgress / 2;
}

int PhysicsEffects::softStart()
{
	if (duration == 0)
		return fullProgress;
	...
	if (span > duration)
		return fullProgress; // якщо минуло більше часу ніж триває розгін - повертаємо максимальний прогрес
	long progress = span * fullProgress / duration;
	return (progress * progress) / fullProgress;
}
int PhysicsEffects::softEnd()
{
	if (duration == 0)
		return fullProgress;
	long current = millis();  // поточний час
...
int PhysicsEffects::softStartSoftEnd()
{
	if (duration == 0)
		return fullProgress;
	long current = millis();  // поточний час
...
MotorBase::MotorBase(String name, PhysicsEffects *effect)
{
	this->name = name;
	this->effect = effect;
}
...
void MotorBase::loop()
{
	...
	if (startup != 0 && ((millis() - startup) < startUpDuration))
	{
		if (delta > 0)
		{
			newSpeed = targetSpeed - delta + startUpSpeed;
		}
		else
		{
			newSpeed = targetSpeed - delta - startUpSpeed;
	...
	else
	{
		if (startup != 0)
		{
			int currentTrget = targetSpeed;
			startup = 0;
			if (delta > 0)
			{
				targetSpeed = targetSpeed - delta + startUpSpeed;
			}
			else
			{
				targetSpeed = targetSpeed - delta - startUpSpeed;
			}
			setSpeed(currentTrget);
		}
...
HBridge::HBridge(String name, int pwmPin, int reley1Pin, int reley2Pin, PhysicsEffects *effect) : MotorBase(name, effect)
{
...
HBridge::HBridge(String name, int pinA, int pinB, PhysicsEffects *effect) : HBridge(name, 0, pinA, pinB, effect)
{
}
...
SpeedController::SpeedController(String name, int pin, PhysicsEffects *effect) : MotorBase(name, effect)
{
	this->pin = pin;
	pinMode(pin, OUTPUT);
	servo = new Servo();
